...
async def main(room: rtc.Room) -> None:
    ...
    print("connecting to room: " + room.name)
...
async def frame_loop(video_stream: rtc.VideoStream) -> None:
    ...
    async for frame_event in video_stream:
        ...
        # frame_event.frame.
        print("Frame info: " + str(frame_event.frame))
        print("Timestamp: " + str(frame_event.timestamp_us))
        buffer = frame_event.frame
        current_timestamp = frame_event.timestamp_us
        if current_timestamp <= previous_timestamp:
            current_timestamp = previous_timestamp + 1  # Increment the timestamp
        ...
        detection_result = landmarker.detect_for_video(mp_image, current_timestamp)
...
if __name__ == "__main__":
    logging.basicConfig(
        level=logging.INFO,
        handlers=[
            logging.FileHandler("logs/face_landmark.log"),
